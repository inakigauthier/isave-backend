...
RSpec.describe 'Api::V1::Portfolios', type: :request do
  ...
  let!(:portfolio) do
    Portfolio.create!(label: 'Prudent Portfolio', portfolio_type: 'CTO', total_amount: 100_000, customer: customer)
  end
  let!(:investment) do
    Investment.create!(isin: 'FR0000000001', investment_types: 'ETF', label: 'Amundi', price: 100.0, sri: 5)
  end
  let!(:portfolio_investment) do
    PortfolioInvestment.create!(portfolio: portfolio, investment: investment, amount_invested: 10_000)
  end
  ...
  describe 'GET /api/v1/customers/:id/portfolios' do
    context 'when the customer exists' do
      ...
      it 'includes investments and their details' do
        ...
        expect(investment_data['share']).to eq((portfolio_investment.amount_invested.to_f / portfolio.total_amount) * 100)
      end
    end
  ...
  describe 'POST /api/v1/customers/:customer_id/portfolios/:id/deposit' do
    ...
    context 'when portfolio type is not allowed' do
      let!(:portfolio) do
        Portfolio.create!(label: 'Other', portfolio_type: 'Other', total_amount: 100_000, customer: customer)
      end
  ...
  describe 'POST /api/v1/customers/:customer_id/portfolios/:id/withdraw' do
    context 'with valid withdrawal' do
      it 'reduces the amount invested and portfolio total amount' do
        ...
        expect(json['total_portfolio_amount'].to_i).to eq(95_000)
      end
    end
  ...
  describe 'POST /api/v1/customers/:customer_id/portfolios/:id/arbitrate' do
    ...
    let!(:portfolio) do
      Portfolio.create!(label: 'CTO Portfolio', portfolio_type: 'CTO', total_amount: 100_000, customer: customer)
    end

    let!(:investment_a) do
      Investment.create!(isin: 'FR0000000001', investment_types: 'ETF', label: 'A', price: 100, sri: 3)
    end
    let!(:investment_b) do
      Investment.create!(isin: 'FR0000000002', investment_types: 'ETF', label: 'B', price: 100, sri: 4)
    end
    ...
    context 'when portfolio type is not allowed' do
      let!(:restricted_portfolio) do
        Portfolio.create!(label: 'PER Portfolio', portfolio_type: 'PER', total_amount: 50_000, customer: customer)
      end
      let!(:pi1) do
        PortfolioInvestment.create!(portfolio: restricted_portfolio, investment: investment_a, amount_invested: 10_000)
      end
      let!(:pi2) do
        PortfolioInvestment.create!(portfolio: restricted_portfolio, investment: investment_b, amount_invested: 10_000)
      end
