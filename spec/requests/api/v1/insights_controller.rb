...
RSpec.describe 'Insights API', type: :request do
  describe 'GET /api/v1/customers/:id/insights' do
    ...
    let(:investment_etf) { Investment.create!(isin: "FR0000000001", investment_types: 'ETF Monde', label: "Amundi", sri: 3, investment_types: 'ETF',price: 100.0) }
    let(:investment_stock) { Investment.create!(isin: "FR0000000002", investment_types: 'Action Tech', sri: 6, label: "Amundi", investment_types: 'Stock', price: 100.0) }

    let(:portfolio1) { Portfolio.create!(label: 'Portefeuille A', total_amount: 30000, customer: customer, portfolio_type: "CTO") }
    let(:portfolio2) { Portfolio.create!(label: 'Portefeuille B', total_amount: 20000, customer: customer, portfolio_type: "CTO") }

    before do
      PortfolioInvestment.create!(portfolio: portfolio1, investment: investment_etf, amount_invested: 10000)
      PortfolioInvestment.create!(portfolio: portfolio1, investment: investment_stock, amount_invested: 20000)

      PortfolioInvestment.create!(portfolio: portfolio2, investment: investment_etf, amount_invested: 20000)
    end
